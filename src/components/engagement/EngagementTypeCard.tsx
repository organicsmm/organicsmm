  ...
  return (
    <Card className={cn(
      ...
      <CardContent className="p-3 sm:p-5">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <div className={cn(
              ...
              <Icon className={cn(
                ...
                config.enabled ? "text-background" : "text-muted-foreground"
              )} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {type === 'views' && (
                  <Badge className="text-[10px] sm:text-xs bg-foreground text-background font-bold px-2 py-0.5">
                    Base
                  </Badge>
                )}
              </div>
              {config.enabled && scheduleData && (
                <div className="flex items-center gap-1.5 sm:gap-2.5 text-xs sm:text-sm text-muted-foreground mt-0.5 sm:mt-1 flex-wrap">
                  <Sparkles className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-foreground shrink-0" />
                  <span className="font-medium">{scheduleData.runCount} runs</span>
                  <span>•</span>
                  <Clock className="h-3 w-3 sm:h-3.5 sm:w-3.5 shrink-0" />
                  <span className="font-medium">~{formatDuration(scheduleData.duration)}</span>
                </div>
              )}
            </div>
          </div>

          {/* Right side: Input + Price + Switch */}
          <div className="flex items-center gap-2 sm:gap-4 justify-between sm:justify-end">
            {config.enabled && (
              <Input
                ...
                className={cn(
                  "w-20 sm:w-28 h-9 sm:h-10 text-sm sm:text-base text-right bg-secondary border-2 border-border text-foreground font-bold",
                  hasError && "border-foreground"
                )}
              />
            )}
            <Badge variant="outline" className="font-mono text-xs sm:text-sm border-2 border-border text-foreground font-bold px-2 sm:px-3 py-1 sm:py-1.5 shrink-0">
        ...
        {/* Quantity Limits - Compact on mobile */}
        {config.enabled && (
          <div className="mt-2 sm:mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 text-xs sm:text-sm text-muted-foreground">
            ...
            {hasError && (
              <div className="flex items-center gap-1.5 text-foreground font-medium">
                ...
                <span className="text-xs sm:text-sm">
                  {isBelowMin && `Min ${providerMin}`}
                  {isAboveMax && `Max ${providerMax.toLocaleString()}`}
                </span>
              </div>
            )}
          </div>
        ...
        {/* Advanced Settings - Compact on mobile */}
        {config.enabled && !hasError && (
          <div className="mt-4 sm:mt-5 pt-4 sm:pt-5 border-t-2 border-border space-y-4 sm:space-y-5">
            {/* Time Limit */}
            <div className="space-y-2 sm:space-y-3">
              <Label className="text-xs sm:text-sm font-bold flex items-center gap-2 text-foreground">
                <Timer className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-foreground" />
                ...
                {TIME_PRESETS.map(preset => {
                  ...
                  return (
                    <Button
                      ...
                      size="sm"
                      className={cn(
                        "h-8 sm:h-9 text-xs sm:text-sm px-2 sm:px-4 font-bold",
                        isSelected 
                          ? "bg-foreground text-background" 
                          : "bg-secondary text-foreground border-2 border-border hover:bg-muted hover:border-foreground/50"
                      )}
            ...
            {/* Variance Slider with Bot Detection Risk */}
            <div className="space-y-3 sm:space-y-4">
              <Label className="text-sm sm:text-base font-bold flex items-center justify-between text-foreground">
                <span className="flex items-center gap-1.5 sm:gap-2">
                  <Shuffle className="h-4 w-4 sm:h-5 sm:w-5 text-foreground" />
                  <span className="text-xs sm:text-base">Random Variance</span>
                </span>
                <span className={cn(
                  "font-mono text-base sm:text-xl font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg transition-colors",
                  variancePercent <= 15 
            ...
            {/* Peak Hours Toggle - Compact on mobile */}
            <div className="flex items-center justify-between p-3 sm:p-4 rounded-xl bg-secondary border-2 border-border gap-3">
              <div className="min-w-0">
                <Label className="text-xs sm:text-sm font-bold flex items-center gap-1.5 sm:gap-2 text-foreground">
                  <Flame className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-foreground shrink-0" />
                  Peak Hours Boost
                </Label>
                <p className="text-[10px] sm:text-sm text-muted-foreground mt-0.5 sm:mt-1 truncate">
                  More during 6-11 PM IST
                </p>
              </div>
              <Switch
                checked={peakHoursEnabled}
                onCheckedChange={handlePeakHoursChange}
              />
            </div>
